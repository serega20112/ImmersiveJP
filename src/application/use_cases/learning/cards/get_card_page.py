...
from src.application.dto.learning import TrackCardPageDTO
from src.application.exceptions import CardNotFoundError
...
class GetCardPageUseCase:
    ...
    async def execute(
        ...
        async with self._uow as uow:
            ...
            batch_cards = await content_repository.list_cards_by_batch(
                user_id,
                track,
                int(card.batch_number),
            )
            batch_completed_ids = await self._get_completed_ids(
                progress_repository, user_id, batch_cards
            )
            ...
            all_current_batch_completed = await self._is_current_batch_completed(
                ...
                card_batch=int(card.batch_number),
    ...
    async def _is_current_batch_completed(
        ...
        *,
        ...
        current_cards = await content_repository.list_cards_by_batch(
            user_id,
            track,
            current_batch,
        )
        current_completed_ids = await self._get_completed_ids(
            progress_repository, user_id, current_cards
        )
